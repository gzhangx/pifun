    ...
    setFreeFormMsgListener(msg => {
        ...
        switch (msg.type) {
            case 'setMasterPlayer':
                ...
                break;
            case 'userInputMsg':
                const { mouse, pid, curBuildType, curKey } = msg;
                ...
                if (!playerState.mouse) playerState.mouse = mouse;
                    ...
                    if (mouse.state) playerState.mouse.state = mouse.state;
                    const mc = mouse.cur;
                    if (mc) {
                        playerState.mouse.cur = mc;                        
                    }
                }
                if (curBuildType) playerState.curBuildType = curBuildType;
                if (curKey) playerState.curKey = curKey;                
                break;
            case 'objSyncMsg':
